...
@router.patch("/{category_id}/",summary="Partial category update",status_code=status.HTTP_200_OK,responses={
    401: {"description":"Не авторизован"},
    403: {"description":"Категория не найдена"},
    415: {"description":"Некорректный формат файла"},
})
async def partial_category_update(category_id:int,title:Optional[str]=Form(None),img:Optional[UploadFile] = None,session:AsyncSession = Depends(get_session),_=Depends(auth.check_if_auth)):
