...
@router.post('/{tg_id}/', summary = "Add Favorite", status_code = status.HTTP_201_CREATED,
            responses={
                400:{"model": ErrorResponse, "description": "Отсутствие такого товара в бд или он уже имеется в избранном у этого пользователя"
                     }
 })
async def add_favorite(tg_id: int, product_id: int, session: AsyncSession = Depends(get_session)):
    return await favorites_services.add_favorite(tg_id, product_id,  session)

@router.delete("/{tg_id}/", summary = "Delete favorite", status_code = status.HTTP_204_NO_CONTENT,
               responses={
                   404:{"model": ErrorResponse, "description": "Отсутствие такого товара у пользователя" }
               })
async def delete_favorite(tg_id: int, product_id: int, session: AsyncSession = Depends(get_session)):
