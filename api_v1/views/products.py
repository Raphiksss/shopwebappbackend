...
@router.post(
    ...
    responses={
        401: {"model": general.ErrorResponse, "description": "Не авторизован"},
        404: {
            "model": general.ErrorResponse,
            "description": "Нарушен FK(Такой категории не существует)",
        },
        413: {"model": general.ErrorResponse, "description": "Файл слишком большой"},
        415: {
            "model": general.ErrorResponse,
...
async def create_product(
    text_data: ProductCreate = Depends(text_data),
    ...
    return await products_services.create_product(
        text_data, image_url, file_location, session
    )
...
@router.get(
    ...
    summary="Get product",
    status_code=status.HTTP_200_OK,
    responses={404: {"model": general.ErrorResponse, "description": "Товар отсутвует"}},
)
...
@router.patch(
    ...
    status_code=status.HTTP_200_OK,
    responses={
        404: {"model": general.ErrorResponse, "description": "Товар отсутвует"},
        401: {"model": general.ErrorResponse, "description": "Не авторизован"},
        413: {"model": general.ErrorResponse, "description": "Файл слишком большой"},
    },
)
async def product_partial_update(
    product_id: int,
    text_data: ProductUpdateStrings = Depends(text_data_partial),
    ...
    if data:
        ...
        with open(data_url, "wb") as f:
            content = await data.read()
            f.write(content)
    return await products_services.partial_update_product(
        product_id=product_id,
        new_products_data=text_data,
        image_url=image_url,
        data_url=data_url,
        session=session,
    )
