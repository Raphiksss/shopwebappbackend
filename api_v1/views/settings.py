...
from api_v1.schemas.settings import AssentColor, StarsExchangeRate, BotToken,CryptoBotToken,YooMoneyData,PaymentMethods
from api_v1.services.auth import check_if_auth
from api_v1.services.settings import change_accent_color,get_accent_color,change_stars_exchange_rate,get_stars_exchange_rate,change_bot_token,get_bot_token,change_crypto_bot_token,get_crypto_bot_token,change_yoomoney_data,get_yoomoney_data,change_payment_methods,get_payment_methods
...
async def set_stars_exchange_rate(exchange_rate:StarsExchangeRate,_=Depends(check_if_auth)):
...
@router.get('/bot_token/',summary="Get bot token",responses=
{
    401:{"model":ErrorResponse,"description":"Не авторизован"}
})
...
@router.post('/crypto_token/',summary="Set crypto_bot token",responses=
{
    401:{"model":ErrorResponse,"description":"Не авторизован"}
})
async def set_crypto_bot_token(crypto_bot_token:CryptoBotToken,_=Depends(check_if_auth)):
    return await change_crypto_bot_token(crypto_bot_token.crypto_bot_token)

@router.get('/crypto_token/',summary="Get crypto_bot token",responses=
{
    401:{"model":ErrorResponse,"description":"Не авторизован"}
})
...
@router.post("/yoo_money/",summary="Set yoomoney data",responses=
{
    401:{"model":ErrorResponse,"description":"Не авторизован"}
})
...
@router.get("/yoo_money/",summary="Get yoomoney data",responses=
{
    401:{"model":ErrorResponse,"description":"Не авторизован"}
})
...
@router.post("/payment_methods/",summary="Set payment_methods",responses=
{
    401:{"model":ErrorResponse,"description":"Не авторизован"}
})
...
@router.get("/payment_methods/",summary="Get payment_methods",response_model=PaymentMethods)
