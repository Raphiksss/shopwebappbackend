...
@router.delete("/{tg_id}/", summary = "Удалить товар с корзины",
               status_code = status.HTTP_204_NO_CONTENT,responses = {
                404:
                    {"model":ErrorResponse, "description": "Нету такого товара в корзине"}
    } )
async def delete_product_from_cart(tg_id:int, prod_id: int):
    return cart_services.del_product(tg_id = tg_id, prod_id = prod_id)


@router.delete("/all_cart/{tg_id}/", summary = "Удалить все с корзины", status_code = status.HTTP_204_NO_CONTENT)
async def delete_cart(tg_id:int):
    return cart_services.del_all_card(tg_id)

@router.delete("/full_product/{tg_id}/", summary = "Удалить полностью один товар с корзины", status_code = status.HTTP_204_NO_CONTENT)
