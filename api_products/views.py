...
@router.post('/', summary = "Создать товар с изображением")
...
@router.get('/', summary = "Получение всех товаров", response_model = list[ProductRead])
async def get_products(session: AsyncSession = Depends(get_session)):
    products = await get_products_crud(session)
    return products

@router.get('/{product_id}/', summary = "Получить товар по ID ", response_model = ProductRead )
