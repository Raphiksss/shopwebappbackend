...
async def stars_buying(tg_id: int, amount: int):
    ...
    await bot.send_invoice(
        ...
        currency="XTR",
        prices=[
            LabeledPrice(label="Цена", amount=int(amount * float(stars_exchange_rate)))
        ],
...
async def get_pay_link(amount):
    ...
    response = requests.post(
        "https://pay.crypt.bot/api/createInvoice", headers=headers, json=data
    )
    if response.ok:
...
async def crypto_replenishment(tg_id: int, amount: int):
    ...
    if pay_link and invoice_id:
        ...
        await bot.send_message(
            chat_id=chat_id,
            text=f"💰 Пополнение баланса на {us_amount} USDT\n\n"
        ...
        await bot.send_message(
            chat_id=chat_id, text="❌ Ошибка при создании счета. Попробуйте позже."
        )
    await bot.session.close()
...
async def successful_payment_handler(message: types.Message):
    ...
    async for session in get_session():
        await users_repository.add_balance(
            message.from_user.id, rubles_amount, session=session
        )
    await message.answer(f"🥳Спасибо за пополнения баланса на {rubles_amount}₽\n")
...
async def include_order(
    tg_id: int, username: str, order_id: int, items: dict, sum: int
):
    bot = await get_bot()
...
async def cmd_start(message: types.Message):
    user = User(
        tg_id=message.from_user.id, username=message.from_user.username, balance=0
    )
    async for session in get_session():
        try:
...
async def admin_panel(message: types.Message):
    keyboard = types.InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="Панель администратора", url=f"https://redstoreapp.com/admin"
                )
            ]
        ]
    )
...
async def check_payment_callback(callback: types.CallbackQuery):
    ...
    if status == "paid":
        ...
        async for session in get_session():
            await users_repository.add_balance(
                callback.from_user.id, rubles_amount, session=session
            )
            break
        ...
        await callback.answer(
            "⏳ Платеж еще не поступил. Попробуйте позже.", show_alert=True
        )
    elif status == "expired":
        await callback.answer("❌ Счет истек. Создайте новый.", show_alert=True)
    else:
        await callback.answer(
            "❌ Не удалось проверить статус платежа.", show_alert=True
        )
...
async def cmd_profile(message: types.Message):
    async for session in get_session():
        profile = await users_repository.get_user(
