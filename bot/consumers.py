...
async def handle_instant_delivery(message: InstantDeliveryMessage):
    try:
        logger.debug(f"Processing instant delivery for order {message.order_id}, user {message.tg_id}")
        ...
        logger.info(f"Successfully delivered product '{message.product_title}' to user {message.tg_id}")
...
async def handle_order_notification(message: OrderNotificationMessage):
    try:
        logger.debug(f"Processing order notification for order {message.order_id}, user {message.tg_id}")
        ...
        logger.info(f"Successfully sent order notification for order {message.order_id}")
...
async def handle_stars_replenishment(message: dict):
    try:
        ...
        logger.debug(f"Processing stars replenishment for user {tg_id}, amount {amount}")
        ...
        logger.error(
            f"Failed to replenish stars for user {tg_id}: {e}",
            exc_info=True
        )
        # Пробрасываем исключение - RabbitMQ автоматически переотправит сообщение (nack)
        # После нескольких попыток сообщение попадет в DLQ
...
async def handle_crypto_bot_replenishment(message: dict):
    try:
        tg_id = message.get("tg_id")
        amount = message.get("amount")
        logger.debug(f"Processing ccrypto_bot replenishment for user {tg_id}, amount {amount}")
        ...
        logger.error(
            f"Failed to replenish crypto_bot for user {tg_id}: {e}",
            exc_info=True
        )
        # Пробрасываем исключение - RabbitMQ автоматически переотправит сообщение (nack)
        # После нескольких попыток сообщение попадет в DLQ
